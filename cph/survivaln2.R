...
library(rms)
library(Cairo)
S5 <- with(data_symbol_cli_N, Surv(Survtime%/%30,Surv==1))
S4 <- with(data_symbol_N_cli, Surv(Survtime%/%30,Surv==1))
...
mudata=data_symbol_cli_N[,variable]
...
median1=median(data_symbol_cli_N[ID1,"Survtime"])
median2=median(data_symbol_cli_N[ID2,"Survtime"])
...
kmfit1<-npsurv(S5~result,data=data_symbol_cli_N)
survplot(kmfit1, conf='none',col=2:1,label.curves=FALSE,xlab='survival time in month',ylab='survival probabilities')
legend("topright",col=2:1,risk,lwd=5)
...
da3<-1-as.numeric(rcorr.cens(predict(f2,newdata=data_symbol_N_cli), S4)[1])
