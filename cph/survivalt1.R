...
S4 <- with(data_symbol_cli_T, Surv(Survtime,Surv==1))
...
mudata=data_symbol_T_cli[,variable]
...
median1=median(data_symbol_T_cli[ID1,"Survtime"])
median2=median(data_symbol_T_cli[ID2,"Survtime"])
...
kmfit1<-npsurv(S5~result,data=data_symbol_T_cli)
...
da3<-1-as.numeric(rcorr.cens(predict(f2,newdata=data_symbol_cli_T), S4)[1])
