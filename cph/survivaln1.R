...
library(rms)
library(Cairo)
S5 <- with(data_symbol_N_cli, Surv(Survtime,Surv==1))
S4 <- with(data_symbol_cli_N, Surv(Survtime,Surv==1))
# dang yingshu fengxi.R
variable<-unlist(strsplit(N, split="+",fixed=T))
P = rep(NA,length(variable))
for(i in 1:length(variable)){
  P[i] = anova(cph(as.formula(paste("S5~",variable[i])),data=data_symbol_N_cli))[5]
  d1<-data_symbol_N_cli[,variable[i]]
  ...
  kmfit1<-npsurv(S5~group,data=data_symbol_N_cli)
...
mudata=data_symbol_N_cli[,variable]
...
f2<-cph(as.formula(paste("S5~",N)),surv=TRUE,x=TRUE,y=TRUE,data=data_symbol_N_cli)
f1<-anova(f2)
as.numeric(S5)
ID1 = names(result)[result==1]
ID2 = names(result)[result==2]
median1=median(data_symbol_N_cli[ID1,"Survtime"])
median2=median(data_symbol_N_cli[ID2,"Survtime"])
...
kmfit1<-npsurv(S5~result,data=data_symbol_N_cli)
...
plot(f1)
da<-validate(f2,data=data_symbol_N_cli,B=200,dxy=TRUE)
write.csv(round(da,4),file = "da.csv")


da2<-1-as.numeric(rcorr.cens(predict(f2), S5)[1])
da3<-1-as.numeric(rcorr.cens(predict(f2,newdata=data_symbol_cli_N), S4)[1])
